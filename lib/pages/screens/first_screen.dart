...
class FirstScreen extends StatelessWidget {
  ...
  Future<void> saveAnswer(BuildContext context, String answer) async {
    if (user != null) {
      final CollectionReference answersCollection =
          FirebaseFirestore.instance.collection('frog_answers');
    ...
    try {
      ...
      if (documents.isNotEmpty) {
        ...
        for (final QueryDocumentSnapshot document in documents) {
          ...
          if (documentData.containsKey('frog_answers')) {
            final dynamic frogAnswersData = documentData['frog_answers'];

            if (frogAnswersData is String) {
              final int? score = int.tryParse(frogAnswersData);
              if (score != null) {
                totalScore += score;
                documentCount++;
  ...
  Widget _questionWidget() {
    return const Text('How would you rate frogs out of 10?');
  }

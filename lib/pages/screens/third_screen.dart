...
class ThirdScreen extends StatelessWidget {
  ...
  Future<void> saveAnswer(BuildContext context, String answer) async {
    if (user != null) {
      final CollectionReference answersCollection =
          FirebaseFirestore.instance.collection('dog_answers');
    ...
    try {
      ...
      if (documents.isNotEmpty) {
        ...
        for (final QueryDocumentSnapshot document in documents) {
          ...
          if (documentData.containsKey('dog_answers')) {
            final dynamic dogAnswersData = documentData['dog_answers'];

            if (dogAnswersData is String) {
              final int? score = int.tryParse(dogAnswersData);
              if (score != null) {
                totalScore += score;
                documentCount++;
  ...
  Widget _questionWidget() {
    return const Text('How would you rate dogs out of 10?');
  }

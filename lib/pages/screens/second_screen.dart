...
class SecondScreen extends StatelessWidget {
  ...
  Future<void> saveAnswer(BuildContext context, String answer) async {
    if (user != null) {
      final CollectionReference answersCollection =
          FirebaseFirestore.instance.collection('cat_answers');
    ...
    try {
      ...
      if (documents.isNotEmpty) {
        ...
        for (final QueryDocumentSnapshot document in documents) {
          ...
          if (documentData.containsKey('cat_answers')) {
            final dynamic catAnswersData = documentData['cat_answers'];

            if (catAnswersData is String) {
              final int? score = int.tryParse(catAnswersData);
              if (score != null) {
                totalScore += score;
                documentCount++;
  ...
  Widget _questionWidget() {
    return const Text('How would you rate cats out of 10?');
  }
